...
namespace snet
{
    ...
    */
    ...
    struct NetUrl
    {
        ...
        std::string host;
        uint16_t port = 0;
        NetUrl(const std::string &url = "")
        {
            parse(url);
        }

        NetUrl(const std::string &proto, const std::string &host, uint16_t port = 0)
        {
            protocol = proto;
            this->host = host;
            this->port = port;
        ...
        //    std::string  get_value(const std::string & val ) const{
        //        return val;
        //    }
        //
        //    template <typename T, typename std::enable_if_t<std::is_same<T, float>::value >* = nullptr>
        //    T  get_value(const std::string & val  ) const{
        //        return std::stof(val);
        //    }
        //
        //    template <typename T, typename std::enable_if_t<std::is_same<T, double>::value >* = nullptr>
        //    T  get_value(const std::string & val  ) const{
        //        return std::stod(val);
        //    }
        //
        //    template <typename T, typename std::enable_if_t<std::is_same<T, long double>::value >* = nullptr>
        //    T  get_value(const std::string & val  ) const{
        //        return std::stold(val);
        //    }
        //
