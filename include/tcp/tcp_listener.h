...
namespace snet
{
	namespace tcp
	{
		...
		template <class Connection, class Factory = TcpFactory<Connection>, class Worker = SNetWorker>
		...
		public:
			...
			TcpListener(Factory *factory, TcpWorkerPtr lisWorker , std::vector<TcpWorkerPtr> workers = {})
			{
			...
			int start(const std::string &url, const snet::NetOptions &netOpts = {})
			{
				NetUrl netUrl;
				netUrl.parse(url);
			...
			void add_worker(TcpWorkerPtr worker)
			{
				if (worker)
				{
